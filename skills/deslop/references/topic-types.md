...
- **Prerequisites** (if any) as a bulleted list. Use the heading `Prerequisites:` (always plural). Don't write `Ensure that...` or `You must have...` — the list is the assertion.
...
- **Place troubleshooting topics at the end of a page.** If there are five or more, move them to a separate page named `<feature>_troubleshooting.md`.
...
- **"Before you begin" section** for prerequisites — friendlier framing than the `Prerequisites:` of a task.
